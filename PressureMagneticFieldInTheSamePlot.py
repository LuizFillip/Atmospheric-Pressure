...
from PressureAnalysis.removeLines import *
...
Plot(files, pre_infile, mag_infile, save = False, 
         nrows = 4, fontsize = 15, ncols = 2)


def Plot(files, pre_infile, mag_infile, save = False, 
         nrows = 4, fontsize = 15, ncols = 2):
    
    fig, axs = plt.subplots(figsize = (12, 10), 
                          nrows = nrows, 
                          ncols = ncols, sharex = True)
    ...
    for num, ax in enumerate(axs.flat):
        ...
        if '15jan.22m' in filename:
            ...
            name = pr.infos_met(filename).infos[0]
    ...
    fig.text(0.92, 0.5, 'Pressure variation (mbar)', va='center', 
                 rotation='vertical', fontsize = fontsize)   
    
    fig.text(0.45, 0.08, 'Universal time (UT)', va='center', 
                 rotation='horizontal', fontsize = fontsize) 
    
    fig.suptitle('dTrend Analysis - 15/01/2022', 
                 y = 0.94, fontsize = fontsize + 5)
    
    if save:
        # Relative path 
        path_to_save = 'PressureAnalysis/Figures/'
        name_to_save = 'MagnetometersPressureDtrend.png'
        plt.savefig(path_to_save + name_to_save, 
